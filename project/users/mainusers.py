...
def getContentUsers(self):
    ...
    outt = subprocess.Popen('last --time-format short', stderr=subprocess.PIPE, stdout=subprocess.PIPE,
    ...
    outtt = subprocess.Popen('lastb --time-format short', stderr=subprocess.PIPE, stdout=subprocess.PIPE,
                             shell=True).communicate()[0].decode('utf-8')
    pp = outtt.split('\n')
...
def updateLoggedInList(self):
    try:
        ...
        o = subprocess.Popen('who -uH', stderr=subprocess.PIPE, stdout=subprocess.PIPE, shell=True).communicate()[
            0].decode()
        l = o.split('\n')
        for i in l:
...
def createLoggedInList(self):
    ...
    self.listLoggedOn.setStyleSheet(
        "color: #2c3e50; selection-background-color: #e74c3c ;background-color: white ; selection-color: #ecf0f1 ;border: 0px solid #95a5a6")
    o = subprocess.Popen('who -uH', stderr=subprocess.PIPE, stdout=subprocess.PIPE, shell=True).communicate()[
        0].decode()
    l = o.split('\n')
    for i in l:
...
def retrievedatafrompasswdfile():
    ...
    for i in list_of_users:
        if int(i[2]) >= 1000:
            ...
            b = subprocess.Popen(c, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()[0].decode(
                'utf-8').split('=', 1)[1]
            c2 = f"id {i[0]} | awk " + "{'print $3'}"
            b2 = \
                subprocess.Popen(c2, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()[
                    0].decode(
                    'utf-8').split('=', 1)[1]
            c3 = f"chage -l {i[0]} | grep 'Account' "
            b3 = \
                subprocess.Popen(c3, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()[
                    0].decode(
                    'utf-8').split(":", 1)[1].strip(" ")
...
class AdministratorCellInTableUsers(QWidget):
    ...
    def setAdmin(self):
        try:
            subprocess.run(f'usermod -aG wheel {self.username}', stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL,
                           shell=True, check=True)
        except subprocess.CalledProcessError as e:
            QMessageBox.critical(self, 'error',
                                 f'error occured during setting this username {self.username} an adminstrator')
            self.slider.setValue(0)
        else:
    ...
    def setNormalUser(self):
        try:
            subprocess.run(f'gpasswd -d {self.username} wheel', stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL,
                           shell=True, check=True)
        except subprocess.CalledProcessError as e:
            QMessageBox.critical(self, 'error',
                                 f'error occured during setting this username {self.username} a normal user')
            self.slider.setValue(1)
        else:
...
class LockCellInTableUsers(QWidget):
    ...
    def setLocked(self):
        try:
            subprocess.run(f'usermod -L {self.username}', stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL,
                           shell=True, check=True)
        except subprocess.CalledProcessError as e:
    ...
    def setUnlocked(self):
        try:
            subprocess.run(f'usermod -U {self.username}', stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL,
                           shell=True, check=True)
        except subprocess.CalledProcessError as e:
