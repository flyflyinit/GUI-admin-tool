...
def updateHostnameUpTimeAndLoadAvgLabell(self):
    ...
    time = subprocess.Popen("date | awk {'print $5'}", shell=True, stdout=subprocess.PIPE,
                            stderr=subprocess.PIPE).communicate()[0].decode('utf-8').replace("\n", "")
    date = \
    subprocess.Popen("date --iso-8601", shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()[
...
def shutdownrebootAction(self):
    ...
    if mbox == QMessageBox.Yes:
        if "shutdown" == self.shre.currentText():
            ...
            try:
                subprocess.run(f'systemctl {self.shre.currentText()}', stderr=subprocess.PIPE, stdout=subprocess.PIPE,
                               shell=True)
            except subprocess.CalledProcessError as e:
