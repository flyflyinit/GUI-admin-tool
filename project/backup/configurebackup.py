...
class CreateFullBackupWindow(QWidget):
    ...
    def generateList(self):
        ...
        fullbackupList.append([now, self.editLineFullBackupName.text() + now, self.editLineFullBackupSrc.text(),
                               self.editLineFullBackupDst.text(), excluded])
...
class CreateIncBackupWindow(QWidget):
    ...
    def createincbackup(self, incbackupList):
        ...
        try:
            ...
            c = "tar -cvvpzg " + incbackups[6] + "/" + incbackupList[1] + "/" + incbackupList[1] + ".snar " + " -f " + \
                incbackups[6] + "/" + incbackupList[1] + "/" + incbackupList[2] + ".tar.gz " + incbackups[7] + " " + \
                incbackups[5]
            subprocess.run(c, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, shell=True, check=True)
        except Exception as e:
            ...
            cur.execute(query, (
            incbackupList[1], maxlevel, incbackupList[0], incbackupList[2], incbackups[5], incbackups[6],
            incbackups[7]))
            con.commit()
    ...
    def createnewincbackup(self, incbackupList):
        ...
        try:
            ...
            c = "tar -cvvpzg " + incbackupList[5] + "/" + incbackupList[0] + "/" + incbackupList[
                0] + ".snar " + " -f " + incbackupList[5] + "/" + incbackupList[0] + "/" + incbackupList[
                    3] + ".tar.gz " + incbackupList[6] + " " + incbackupList[4]
            subprocess.run(c, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, shell=True, check=True)
        except Exception as e:
            ...
            query = "INSERT INTO IncrementalBackups (metaname,level,date,name,srcpath,dstpath,excluded) VALUES (?,?,?,?,?,?,?)"
            cur.execute(query, (
            incbackupList[0], incbackupList[1], incbackupList[2], incbackupList[3], incbackupList[4], incbackupList[5],
            incbackupList[6]))
            con.commit()
