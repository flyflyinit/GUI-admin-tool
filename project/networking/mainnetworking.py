...
from project.networking.networkingScripts import currentlyActiveConnectionNow, upConnection, downConnection, \
    AutoConnection
...
def updatetcpudpsocketsList(self):
    try:
        ...
        o = subprocess.Popen('ss -ta', stderr=subprocess.PIPE, stdout=subprocess.PIPE, shell=True).communicate()[
            0].decode()
        l = o.split('\n')
        for i in l:
            i.replace("   ", "")
...
def createtcpudpsocketsList(self):
    ...
    self.listtcpudpsockets.setStyleSheet(
...
def createTableNet(self):
    ...
    self.tableNet.resizeColumnsToContents()
    showmyuserslist(self)
